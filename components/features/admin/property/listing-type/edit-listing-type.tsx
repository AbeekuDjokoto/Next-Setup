...
interface Props {
  ...
  isLoading: boolean;
  selectedType?: any;
  onEditListingType: (...any: any) => any;
}

export function EditListingType({ closeModal, onEditListingType, isLoading, selectedType }: Props) {
  const onSubmit: SubmitHandler<{ name: string }> = (data) => {
    onEditListingType(data);
  };
  return (
    <div className="grid gap-8 bg-white rounded-md w-[450px] max-w-[90%] p-6">
      <div className="flex justify-between items-center ">
        <h2 className="font-semibold text-2xl">Edit Listing Type</h2>
